...
|-------|---------|-------------|
| `/deep-plan` | Full planning pipeline: product review, optional design exploration, spec, deep reviews, multi-review on plan, decomposition — with checkpoints between each phase | New feature or idea needing thorough pre-execution planning. Supports `--yes`, `--skip-reviews`, `--no-decompose`. |
